...
async function testFirstBatch() {
  ...
  console.log('Testing first batch of 500 rows with fixed 2-digit seconds ISO timestamp...');
  ...
  for await (const line of rl) {
    ...
    sampleRows.push({
      id: deliveryNo ? `hist-${logType.toLowerCase()}-${deliveryNo}` : `hist-gen-${sampleRows.length}`,
      log_type:              logType,
  ...
  console.log(`Parsed ${sampleRows.length} sample rows. First row timestamp_start:`, sampleRows[0].timestamp_start);
  const table = bigquery.dataset(DATASET_ID).table(TABLE_ID);
  
  try {
    const res = await table.insert(sampleRows, { skipInvalidRows: true, ignoreUnknownValues: true });
    console.log('Batch insert SUCCESS!', res[0]);
  } catch (err) {
    console.error('Batch insert FAILED:', err ? err.message : err);
    if (err && err.errors) {
      console.error('Error Details:', JSON.stringify(err.errors.slice(0, 3), null, 2));
    }
  }
}
