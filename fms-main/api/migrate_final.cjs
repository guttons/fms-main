...
async function runMigration() {
  ...
  console.log(`==================================================`);
  console.log(`Step 1: Truncating operations_log table in BigQuery...`);
  
  const truncateSql = `TRUNCATE TABLE \`${PROJECT_ID}.${DATASET_ID}.${TABLE_ID}\``;
  await bigquery.query({ query: truncateSql, location: 'US' });
  console.log(`Successfully truncated \`${PROJECT_ID}.${DATASET_ID}.${TABLE_ID}\``);

  console.log(`\nStep 2: Parsing CSV dataset into memory array...`);
  ...
  console.log(`Parsed ${allRows.length} total rows into memory.`);
  console.log(`  - Into-Plane (FLIGHT): ${counts.FLIGHT}`);
  console.log(`  - Seaplane (SEAPLANE): ${counts.SEAPLANE}`);
  console.log(`  - Marine Loading (MARINE): ${counts.MARINE}`);

  console.log(`\nStep 3: Sequential stream insertion into BigQuery in batches of 1,000...`);
  ...
  for (let i = 0; i < allRows.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (totalInserted % 10000 === 0 || i + BATCH_SIZE >= allRows.length) {
        console.log(`[Progress] Batch ${Math.floor(i / BATCH_SIZE) + 1} | Inserted ${totalInserted} / ${allRows.length} rows (${Math.round(totalInserted / allRows.length * 100)}%)`);
      }
    } catch (err) {
      console.error(`[Error] Batch at index ${i} failed:`, err ? err.message : err);
  ...
  console.log(`Inserted into BigQuery: ${totalInserted}`);
  console.log(`Failed rows: ${totalErrors}`);
  console.log(`  - Into-Plane (FLIGHT): ${counts.FLIGHT}`);
  console.log(`  - Seaplane (SEAPLANE): ${counts.SEAPLANE}`);
  console.log(`  - Marine Loading (MARINE): ${counts.MARINE}`);
  console.log(`==================================================`);
