...
export const Dashboard: React.FC<DashboardProps> = ({ tanks, user }) => {
  ...
  // --- Sub-Component: Depot Dashboard (AeroFuel Command Style) ---
  const renderDepotDashboard = () => (
    <div className="space-y-8 fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 card-premium p-8 relative overflow-hidden flex flex-col justify-between min-h-[320px]">
          <div className="relative z-10">
            <p className="label-sm text-slate-400 mb-2">Current Operations</p>
            <h3 className="title-lg text-slate-900 border-l-4 border-primary pl-4">Active Refueling Units</h3>
          ...
          <div className="relative z-10 flex items-baseline space-x-6">
            <span className="text-[120px] font-[900] text-primary leading-none tracking-tighter">8</span>
            <span className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Aircraft on Bay</span>
          </div>

          <div className="relative z-10 flex space-x-1 h-2 w-full max-w-md">
            <div className="flex-1 bg-primary rounded-full"></div>
            <div className="flex-1 bg-primary rounded-full"></div>
            <div className="flex-1 bg-primary rounded-full"></div>
            <div className="flex-1 bg-primary rounded-full"></div>
            <div className="flex-1 bg-slate-200 rounded-full opacity-50"></div>
          </div>

          {/* Abstract Background Element */}
          <div className="absolute -right-20 -bottom-20 w-96 h-96 bg-primary/5 rounded-full blur-3xl"></div>
        </div>

        {/* Critical Alerts Panel */}
        <div className="card-premium bg-error/5 border-error/10 p-8 flex flex-col">
          <div className="flex items-center justify-between mb-8">
             <h3 className="label-sm text-error">Critical Alerts (2)</h3>
             <AlertTriangle className="w-5 h-5 text-error opacity-40" />
          </div>
          <div className="space-y-6 flex-1">
             <div className="bg-white p-6 rounded-2xl border border-error/10 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                <div className="flex justify-between items-start">
                   <div>
                      <p className="text-xs font-black text-error uppercase mb-1">Tank TK-8 Priority</p>
                      <p className="text-sm font-medium text-slate-500">Remaining Capacity: 15%</p>
                   </div>
                   <Clock className="w-4 h-4 text-slate-300 group-hover:text-error transition-colors" />
                </div>
             </div>
             <div className="bg-white p-6 rounded-2xl border border-error/10 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                <div className="flex justify-between items-start">
                   <div>
                      <p className="text-xs font-black text-error uppercase mb-1">System Mismatch</p>
                      <p className="text-sm font-medium text-slate-500">Flow Meter Bay 3 Error</p>
                   </div>
                   <Activity className="w-4 h-4 text-slate-300 group-hover:text-error transition-colors" />
                </div>
             </div>
          </div>
          <button className="w-full mt-6 py-3 text-[10px] font-black uppercase text-error hover:bg-error/5 rounded-xl transition-all tracking-widest">
             Review All Alerts
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="card-premium p-8">
          <p className="label-sm text-slate-400 mb-6">Total Jet A-1 Stock</p>
          <div className="flex items-baseline space-x-3 mb-2">
            <h3 className="headline-lg text-slate-900">13.03M L</h3>
            <span className="text-[10px] font-bold text-success flex items-center">
              <TrendingUp className="w-3 h-3 mr-1" /> +0.2%
            </span>
          </div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Stable vs Yesterday</p>
        </div>

        <div className="card-premium p-8">
          <p className="label-sm text-slate-400 mb-6">Days Cover</p>
          <div className="flex items-baseline space-x-3 mb-2">
            <h3 className="headline-lg text-slate-900">12.5 Days</h3>
            <span className="bg-success/10 text-success text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-tighter ml-4">
              Optimal
            </span>
          </div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Based on 30D Average</p>
        </div>

        <div className="card-premium p-8 overflow-hidden relative">
          <p className="label-sm text-slate-400 mb-6">Operational Efficiency</p>
          <div className="flex items-baseline space-x-3 mb-4">
            <h3 className="headline-lg text-slate-900">94.2%</h3>
            <span className="text-[10px] font-bold text-slate-400 ml-4">Target: 96%</span>
          </div>
      ...
      <div className="card-premium p-10">
        <div className="flex items-center justify-between mb-10">
          <div>
            <h3 className="title-md text-slate-900">Today's Uplift Volume</h3>
            <p className="text-xs font-medium text-slate-400 mt-1">Live comparison: Planned (Target) vs Actual Performance</p>
          </div>
          <div className="flex items-center space-x-8">
             <div className="flex items-center space-x-3">
                <div className="w-4 h-0.5 bg-slate-400 border-t border-dashed border-slate-400"></div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Planned</span>
             </div>
             <div className="flex items-center space-x-3">
                <div className="w-4 h-0.5 bg-primary"></div>
                <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Actual</span>
             </div>
          </div>
        </div>
        <div className="h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={HOURLY_DATA}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="rgba(0,32,70,0.05)" />
              <XAxis 
                dataKey="hour" 
                axisLine={false} 
                tickLine={false} 
              ...
              <Line 
                ...
                strokeWidth={4} 
                dot={{r: 0}} 
                activeDot={{r: 8, stroke: '#fff', strokeWidth: 4, fill: '#002046'}} 
              />
              <Line 
                type="monotone" 
                dataKey="diesel" 
      ...
      <div className="card-premium p-8">
        <h4 className="label-sm text-slate-400 mb-8 tracking-[0.2em]">Asset Grid</h4>
            ...
            return (
              <div key={tank.id} className="group cursor-pointer">
                 <div className="flex justify-between items-center mb-3">
                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-tighter group-hover:text-primary transition-colors">{tank.name}</span>
                    {isLow ? <AlertTriangle className="w-3 h-3 text-error" /> : <div className="w-1.5 h-1.5 bg-success/40 rounded-full"></div>}
                 </div>
                 <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-1000 ${isLow ? 'bg-error' : 'bg-primary'}`} 
                      style={{ width: `${fillPct}%` }}
                    ></div>
                 </div>
                 <p className="text-[10px] font-bold text-slate-400 mt-2">{fillPct.toFixed(0)}% Utilized</p>
              </div>
            );
          })}
        </div>
        ...
        <>
            ...
            {isDualRole && (
                <div className="mb-10 flex justify-start">
                    <div className="bg-surface-dim p-1.5 rounded-2xl border border-outline flex space-x-2">
                        <button
